...
class AnalysisCache:
    ...
    def get(self, path: Path) -> dict[str, Any] | None:
        ...
        with sqlite3.connect(self.database_path, timeout=5.0) as db:
            ...
            row = db.execute(
                ...
                WHERE file_path = ? AND file_size = ? AND modified_ns = ?
                LIMIT 1
                """,
                (file_path, size, modified_ns),
            ).fetchone()

        if not row or not row["analysis_payload"]:
            return None

        try:
            result = json.loads(row["analysis_payload"])
        except (TypeError, ValueError, json.JSONDecodeError):
            return None

        result["cache"] = {
            "hit": True,
            "message": "Unveränderte Datei – gespeichertes Analyseergebnis verwendet.",
        }
        return result
        ...
        with sqlite3.connect(self.database_path, timeout=10.0) as db:
            ...
            db.execute(
                ...
                INSERT INTO identification_cache(
                    file_path, file_size, modified_ns, confidence,
                    method_summary, analysis_payload, analyzed_at
                )
                ...
                (
                    ...
                    modified_ns,
                    float(
                        (result.get("identification") or {}).get("confidence")
                        or 0.0
                    ),
                    ", ".join(result.get("methods_used") or []),
                    payload,
                ),
            )
            db.commit()
    ...
    def _ensure_payload_column(db: sqlite3.Connection) -> None:
        ...
        if "analysis_payload" not in columns:
            db.execute(
                "ALTER TABLE identification_cache "
                "ADD COLUMN analysis_payload TEXT"
            )
            db.commit()
